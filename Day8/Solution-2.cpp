...
struct coordinate {
    ...
    coordinate(size_t x_pos, size_t y_pos, size_t z_pos) {
        x = x_pos;
        y = y_pos;
        z = z_pos;
    }
...
size_t process_circuits(vector<coordinate> coordinates) {
    ...
    return coordinates[last_connection.breaker1].x * coordinates[last_connection.breaker2].x;
}
...
size_t process_file() {
    ...
    return process_circuits(coordinates);
}

int main() {
    size_t output = process_file();
    cout << "The mul of the X coordinates of the last two junction boxes is " << output << endl;
}